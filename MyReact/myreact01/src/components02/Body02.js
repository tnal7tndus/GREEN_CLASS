...
//      -> State변수로 지정된 변수의 값에 변화가 일어나면 리액트에서는 리랜더링 해줌  
...
// ** State로 사용자 입력 관리하기
// => 사용자가 Text를 입력할때마다 console 출력하기 
// => 과제 "-" 버튼 만들기
//    - 최소값은 0 : alert 경고창 출력   
//    - 최대값은 100 : alert 경고창 출력, 0으로 초기화
...
function Body() { 
  ...
  const onIncrease2 = () => {
    if (count>19) {
      ...
      //test2 : count2의 값 5의배수 일때 setCount() 호출
  ...
  const textChange = (e) => {
    ...
    // text = e.target.value; staet와 무관, 반드시 setText(..) 사용
  } //textChange
  // => date
  ...
  //    option의 컨텐츠가 select의 value에 전달됨
  //    ( select Tag에 value 속성을 정의하지 않아도 전달됨 )

  // => props로 option을 동적으로 만드는경우
  //    props로 전달받은 Data_list를 이용해 option을 만드는 경우에는
  //    option의 key, value 속성을 이용함
  ...
  return (
    <div className='body'>
      ...
      <div>
        ...
        <span>{text}</span>
        <p>[연습] input Tag의 type date 활용해서 값이 변경될때 마다 그값을 console로 출력하기</p>
